//! Sideways-pencil workload through the production stroke engine, without a window.
use graphite_studio::{
    core::{
...
fn main() {
    ...
    for (dpi, core) in [(120., 1.5), (240., 5.)] {
        for tilt in [8., 45., 78.] {
            let key = format!("{dpi:.0}-{core:.1}-{tilt:.0}");
            let mut timings = Vec::new();
            let mut expected = None;
            for run in 0..3 {
                let spec = CanvasSpec::from_physical("Pencil benchmark", 85., 65., dpi);
